...
export default function HornButton({ onHonk, lang }) {
  ...
  const handleHornClick = () => {
    ...
    const popupText = hornName ? `🎺 ${hornName}` : (t.hornPopups ? t.hornPopups[Math.floor(Math.random() * t.hornPopups.length)] : "POOO POOO! 🎺");
  ...
  return (
    <div className="fixed left-2 sm:left-8 top-[50%] -translate-y-1/2 z-30 flex flex-col items-center select-none font-odia">
      
      {/* Floating sound effect text popups */}
      {hornPopups.map(popup => (
        <div
          key={popup.id}
          className="absolute -top-14 font-odia text-[10px] sm:text-sm font-black text-amber-300 drop-shadow-[0_4px_12px_rgba(245,158,11,0.8)] pointer-events-none animate-bounce whitespace-nowrap z-40 bg-slate-900/95 border border-amber-400 px-2.5 py-0.5 rounded-full shadow-2xl"
      ...
      >
        ...
        {/* Horn Icon & Localized Sticker Text */}
        ...
        <span className="font-odia text-[10px] sm:text-sm font-black text-amber-100 mt-0.5 tracking-tight drop-shadow-md text-center px-1 truncate max-w-[70px] sm:max-w-none">
          {t.hornSticker}
        </span>

        <Sparkles className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 text-amber-300 animate-spin" />
      </button>

      <span className="mt-1 sm:mt-2 text-[9px] sm:text-[10px] text-slate-300 font-odia bg-slate-950/70 px-2 py-0.5 rounded border border-slate-800 backdrop-blur-sm hidden xs:inline">
        {t.tapToHonk} 🎺
      </span>
    </div>
  );
